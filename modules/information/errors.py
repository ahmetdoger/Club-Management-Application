...
def raise_invalid_age_error(age: int, min_age: int, max_age: int):
  
    msg = f"Girilen yaş ({age}) geçersiz. {min_age} ile {max_age} arasında olmalıdır."
...
def raise_retirement_age_error(name: str, age: int, limit: int = 38):

    msg = f"{name} ({age} yaşında), emeklilik yaşı olan {limit}'i doldurmuştur."
    raise ClubManagerError(msg, "ContractError", 4001)

def raise_insufficient_performance_error(name: str, played: int, required: int):
    msg = f"{name} yetersiz maç sayısı ({played}/{required}) nedeniyle kriterleri sağlamıyor."
    raise ClubManagerError(msg, "PerformanceError", 4002)

def raise_clearance_missing_error(name: str):
    msg = f"Transfer Engeli: {name} için 'Temiz Kağıdı' (Clearance) belgesi eksik."
    raise ClubManagerError(msg, "TransferError", 4003)

def raise_high_penalty_error(points: int, limit: int):
    msg = f"Disiplin Engeli: Ceza puanı ({points}), limiti ({limit}) aşmıştır."
    raise ClubManagerError(msg, "DisciplineError", 4004)

def raise_status_change_error(current_status: str, new_status: str, reason: str):
    msg = f"Statü Değişikliği Reddedildi: {current_status} -> {new_status}. Sebep: {reason}"
